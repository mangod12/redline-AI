...
class EmotionAnalysis(BaseModel):
    ...
    emotion_scores: dict[EmotionType, float] = Field(
        ..., description="Confidence scores for each emotion"
    )
    intensity: float = Field(
        ..., ge=0.0, le=1.0, description="Intensity of the primary emotion"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Overall confidence in the analysis"
    )
    text_segments: list[str] = Field(
        default_factory=list, description="Text segments analyzed"
    )

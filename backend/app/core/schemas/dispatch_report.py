...
class DispatchReport(BaseModel):
    ...
    priority: str = Field(
        ..., description="Priority level (e.g., 'immediate', 'urgent', 'routine')"
    )
    resources_required: list[str] = Field(
        default_factory=list, description="Required resources or services"
    )
    location: str | None = Field(None, description="Location information if available")
    estimated_response_time: str | None = Field(
        None, description="Estimated response time"
    )
    reasoning: str = Field(..., description="Explanation of the dispatch decision")
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="Timestamp of the report"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in the dispatch recommendation"
    )
